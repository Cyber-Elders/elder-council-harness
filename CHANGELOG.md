...
- **Six cyber councils** (pure-data YAML, installable into every supported IDE): Code, Threat Hunting,
  Supply Chain Audit, Multi-Jurisdictional Compliance, Cyber Risk, Platform Architecture.
...
- **Six SME lenses** + specialised roles, with role-based model resolution (`council-models.json`,
  ...
  sentinels).
- **IDE adapters:** Claude Code, OpenCode (hard-block); Kiro (best-effort hard-block); Cursor + any
  MCP client (advisory). Generic renderer — councils are data, no per-council code branches.
