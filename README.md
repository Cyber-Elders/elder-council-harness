...
[![Tests](https://img.shields.io/badge/tests-121%20passing-brightgreen.svg)](docs/TESTING.md)
...
*Multi-model **councils** for high-stakes cyber decisions — convened only when the decision is worth it.*
...
compliance obligations, and scores your risk, its blind spots stop being an isolated bad answer.
They become a **repeatable pattern inside your decision infrastructure** — propagating through teams,
tools, and downstream systems at machine speed. A confident, stale, biased, or manipulated model
becomes a **systemic risk**: a single point of institutional failure.
...
every task. It is a risk-governance pattern for the decisions that matter.
...
$ eldercouncil convene code-council --demo --question "merge a diff with a hardcoded AWS key"
  ...
  code-council  —  action-gate
    ...
    AppSec SME                     block           [HIGH]      ( 0.9)  Hardcoded secret + unparameterised SQL.
    Reliability / Operations SME   merge                       (0.55)  Rollback path exists.
    Deterministic Tool Lens        block           [CRITICAL]  (0.95)  secret-scan: AWS key; SAST: SQLi sink.
    Critic / Challenge             request-changes             ( 0.7)  Auth check is assumed, not verified.

  VERDICT: block   → route: human
...
```console
pip install eldercouncil            # ships no model and no API keys (bring your own LLM)
