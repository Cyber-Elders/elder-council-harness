...
def _doc(name: str, flow):
    ...
    SimpleDocTemplate(str(OUT / name), pagesize=A4,
                      ...
                      title="Elder Council Harness", author="ZenBlue Pty Ltd t/a Cyber Elders").build(flow)
...
def overview(ss):
    ...
    s += [Spacer(1, 10), Paragraph("Apache-2.0 (code) + CC BY 4.0 (docs). © 2026 ZenBlue Pty Ltd t/a Cyber Elders.", ss["ECBody"])]
    return s
